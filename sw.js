...
function isDuplicate(array, field, value, excludeIndex = -1) {
...
class AutosaveManager {
  ...
  trigger() {
      ...
      this.timer = setTimeout(() => this.perform(), this.delay);
  }
  
  async perform() {
      if (!db || !currentInspection || !currentInspection.id || this.isSaving) {
          return;
      }
  ...
  persistToDatabase() {
      return new Promise((resolve, reject) => {
          persistCurrentInspection((success) => {
              success ? resolve() : reject(new Error('Database save failed'));
          });
      });
  }
}
...
function persistCurrentInspection(callback) {
  if (!db || !currentInspection || !currentInspection.empresa) {
      console.error('Cannot persist: missing db or inspection data');
          ...
          <form id="empresa-form" oninput="triggerAutosave()">
              <div class="form-grid">
                  <div class="form-group">
                      ...
                      <div style="display: flex; gap: 0.5rem;">
                          <input type="text" id="nome" required value="${escapeHtml(e.nome) || ''}" placeholder="Ex: Acme Corporation" style="flex-grow: 1;">
                  ...
                  <div class="form-group">
                      ...
                      <div style="display: flex; gap: 0.5rem;">
                          <input type="text" id="cnpj" value="${escapeHtml(e.cnpj) || ''}" placeholder="00.000.000/0000-00" style="flex-grow: 1;">
              ...
              <div class="form-grid">
                  ...
                  <div class="form-group">
                      ...
                      <div style="display: flex; gap: 0.5rem;">
                          <input type="text" id="elaborado" value="${escapeHtml(e.elaborado) || ''}" placeholder="Nome do responsável" style="flex-grow: 1;">
              ...
              <div class="form-group">
                  ...
                  <div style="display: flex; gap: 0.5rem;">
                      <input type="text" id="aprovado" value="${escapeHtml(e.aprovado) || ''}" placeholder="Nome do aprovador" style="flex-grow: 1;">
...
function saveDepartamento() {
  ...
  if (isDuplicate(currentInspection.departamentos, 'nome', deptoData.nome, editingIndex)) {
      return showToast("Já existe um departamento com este nome.", "warning");
  }
...
function saveCargo() {
  ...
  if (isDuplicate(depto.cargos, 'nome', nome, editingType === 'cargo' ? editingIndex : -1)) {
      return showToast("Já existe um cargo com este nome neste departamento.", "warning");
  }
  
  const cargoData = { 
      nome, 
      ...collectFormData('cargo'), 
      riscos: editingIndex > -1 && editingType === 'cargo' ? depto.cargos[editingIndex].riscos : [] 
  };
...
function saveFuncionario() {
  ...
  if (isDuplicate(depto.funcionarios, 'nome', nome, editingType === 'funcionario' ? editingIndex : -1)) {
      return showToast("Já existe um funcionário com este nome neste departamento.", "warning");
  }
  
  const funcionarioData = { 
      nome, 
      ...collectFormData('funcionario'), 
      riscos: editingIndex > -1 && editingType === 'funcionario' ? depto.funcionarios[editingIndex].riscos : [] 
  };
...
function saveGrupo() {
  ...
  const grupoData = { 
      listaDeCargos: nomes, 
      ...collectFormData('grupo'), 
      riscos: editingIndex > -1 && editingType === 'grupo' ? depto.grupos[editingIndex].riscos : [], 
      id: editingIndex > -1 && editingType === 'grupo' ? depto.grupos[editingIndex].id : 'grupo_' + Date.now() 
  };
...
function saveRisco() {
  ...
  if (currentGroupId) {
      ...
      const funcionario = depto.funcionarios[activeFuncionarioIndex]; 
      if (!funcionario.riscos) funcionario.riscos = [];
      targetArray = funcionario.riscos; 
      message += '!';
  }
  if (editingIndex > -1) targetArray[editingIndex] = riscoData; 
  else targetArray.push(riscoData);
...
function editRisco(index) {
  ...
  if (currentGroupId) { 
      const grupo = depto.grupos.find(g => g.id === currentGroupId); 
      risco = grupo?.riscos[index]; 
  }
  else if (activeCargoIndex > -1) { 
      risco = depto.cargos[activeCargoIndex].riscos[index]; 
  }
  else if (activeFuncionarioIndex > -1) { 
      risco = depto.funcionarios[activeFuncionarioIndex].riscos[index]; 
  }
  if (!risco) return;
  ...
  function deleteRisco(index) {
      const depto = currentInspection.departamentos[activeDepartamentoIndex]; 
      let targetArray, nomeRisco;
      if (currentGroupId) { 
          const grupo = depto.grupos.find(g => g.id === currentGroupId); 
          targetArray = grupo.riscos; 
      }
      else if (activeCargoIndex > -1) { 
          targetArray = depto.cargos[activeCargoIndex].riscos; 
      }
      else if (activeFuncionarioIndex > -1) { 
          targetArray = depto.funcionarios[activeFuncionarioIndex].riscos; 
      }
      ...
      request.onsuccess = () => {
          ...
          (insp.departamentos || []).forEach(depto => {
              ...
              (depto.grupos || []).forEach(grupo => {
                  ...
                  html += renderCargoReport(g, `👥 ${escapeHtml(g.nome)}`);
              });
              (depto.cargos || []).forEach(cargo => {
                  html += renderCargoReport(cargo, `👤 Cargo: ${escapeHtml(cargo.nome)||'N/A'}`);
              });
              (depto.funcionarios || []).forEach(func => {
                  html += renderCargoReport(func, `👷 Funcionário: ${escapeHtml(func.nome)||'N/A'}`);
              });
              html += `</div>`;
          });
  ...
  console.log("✅ Sistema carregado com melhorias aplicadas!");